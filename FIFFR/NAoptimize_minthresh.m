% Optimize minthresh
function [minthresh_sampledDiff, optimized_minthresh_value] = optimize_minthresh(a, sigshift, maxthresh, minthresh_list, cyclenumber, Irecon, index, NOriginal, replvalue,  NA, n, lambda, FRt,simtype)
    ...
    % minthresh_list: the value for the minimum threshold
    ...
    switch simtype
        case 'psnr'   
            for ii =1:length(minthresh_list)
                sx = linspace(-10, 10,cyclenumber); 
                pthresh = sigmoid(a, sx, sigshift, maxthresh, minthresh_list(ii)); 
            ...
            minthresh_sampledDiff = cell2mat(AbsSampledDiff); 
            [~, minthreshind] = max(minthresh_sampledDiff); 
            optimized_minthresh_value = minthresh_list(minthreshind); 
